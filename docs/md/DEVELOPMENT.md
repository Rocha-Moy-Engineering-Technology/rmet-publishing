...
`Subscribe.astro` renders the section on the landing page and on every piece page, before the comments: a Subscribe button beside an RSS feed button. The Subscribe button is a `popovertarget` for a `[popover]` card (`.popover` in `global.css`), so the browser opens it in the top layer, focuses the `autofocus` email input, and closes it on Escape, an outside click, or the Close button, with no script. The form inside is plain HTML: `method="post"` to the action with one `type="email"` input named by the field variable, submitted in a new tab so the provider's confirmation page does not replace the piece; a submit button ignores `popovertarget`, so the card stays until dismissed. `logic/subscribe/subscribe_settings.ts` resolves the two variables. `RMET-BDD-006` builds with fixture variables, opens the popup, checks the email box has focus, intercepts the post in the browser, and checks the address lands under the configured field; `RMET-E2E-010` covers the unconfigured state on the production build (RSS feed button, no Subscribe button, no form); `RMET-SMOKE-002` checks the section is served beside the feed.
