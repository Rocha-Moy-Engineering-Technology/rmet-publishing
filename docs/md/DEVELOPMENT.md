...
`FeedIcons.astro` renders an envelope button and an RSS feed icon in the header and footer icon rows of every page, beside GitHub and LinkedIn; `Subscribe.astro`, mounted once by `BaseLayout.astro`, renders the `[popover]` card (`.popover` in `global.css`, id shared through `subscribe_popover.ts`). Each envelope is a `popovertarget` for that card, so the browser opens it in the top layer, focuses the `autofocus` email input, and closes it on Escape, an outside click, or the Close button, with no script. The form inside is plain HTML: `method="post"` to the action with one `type="email"` input named by the field variable, submitted in a new tab so the provider's confirmation page does not replace the piece; a submit button ignores `popovertarget`, so the card stays until dismissed. `logic/subscribe/subscribe_settings.ts` resolves the two variables. `RMET-BDD-006` builds with fixture variables, checks both icon rows, opens the popup from the header envelope, checks the email box has focus, intercepts the post in the browser, and checks the address lands under the configured field; `RMET-E2E-010` covers the unconfigured state on the production build (two RSS feed icons, no envelope, no popup); `RMET-SMOKE-002` checks the feed icon is served beside the feed.
